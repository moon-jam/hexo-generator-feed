...
function composeItemContent(post, feedConfig) {
  ...
  if (content && post.content) {
    // Strip control characters but preserve whitespace (\t, \n, \r) — stripping
    // newlines breaks LaTeX like \<newline>j which pandoc emits for "\ j".
...
function generateAtom(feed, items) {
  ...
  // feedsmith does not emit type="html" on <content> / <summary> elements.
  // Without that attribute, Atom readers default to type="text" and display
  // the raw HTML markup instead of rendering it.
  ...
  return xml
    .replace(/<content(?![^>]*\btype=)([^>]*)>/g, '<content type="html"$1>')
    .replace(/<summary(?![^>]*\btype=)([^>]*)>/g, '<summary type="html"$1>');
}
